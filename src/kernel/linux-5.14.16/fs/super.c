...
bool trylock_super(struct super_block *sb)
{
	if (down_read_trylock(&sb->s_umount)) {
		if (!hlist_unhashed(&sb->s_instances) && sb->s_root &&
		    (sb->s_flags & SB_BORN))
			return true;
		up_read(&sb->s_umount);
	}
	...
	int (*set)(struct super_block *, struct fs_context *))
{
	...
	struct user_namespace *user_ns =
		fc->global ? &init_user_ns : fc->user_ns;
	int err;
...
static void do_emergency_remount_callback(struct super_block *sb)
{
	...
	if (sb->s_root && sb->s_bdev && (sb->s_flags & SB_BORN) &&
		...
		fc = fs_context_for_reconfigure(
			sb->s_root, SB_RDONLY | SB_FORCE, SB_RDONLY);
		if (!IS_ERR(fc)) {
		  ...
		  int (*fill_super)(struct super_block *, struct fs_context *))
{
	...
	if (s->s_root) {
		...
		if ((fc->sb_flags ^ s->s_flags) & SB_RDONLY) {
			warnf(fc, "%pg: Can't mount, would change RO state",
			      bdev);
			deactivate_locked_super(s);
			blkdev_put(bdev, mode);
			return -EBUSY;
