...
__cacheline_aligned_in_smp DEFINE_KOMB_SEQLOCK(rename_lock);
...
static void __dentry_kill(struct dentry *dentry)
{
	 ...
	 */
	komb_lockref_mark_dead(&dentry->d_lockref);
...
static inline bool fast_dput(struct dentry *dentry)
{
	 ...
	 */
	if (unlikely(dentry->d_flags & DCACHE_OP_DELETE))
		return komb_lockref_put_or_lock(&dentry->d_lockref);

	/*
	 * .. otherwise, we can try to just decrement the
	 * lockref optimistically.
	 */
	ret = komb_lockref_put_return(&dentry->d_lockref);
...
{
	komb_lockref_get(&dentry->d_lockref);
}
...
struct dentry *dget_parent(struct dentry *dentry)
{
	...
	gotref = komb_lockref_get_not_zero(&ret->d_lockref);
	rcu_read_unlock();
	if (likely(gotref)) {
...
void shrink_dcache_sb(struct super_block *sb)
{
	do {
		...
		list_lru_walk(&sb->s_dentry_lru, dentry_lru_isolate_shrink,
			      &dispose, 1024);
		shrink_dentry_list(&dispose);
...
int d_set_mounted(struct dentry *dentry)
{
	struct dentry *p;
	int ret = -ENOENT;
	komb_write_seqlock(&rename_lock);
	...
	spin_unlock(&dentry->d_lock);
out:
	komb_write_sequnlock(&rename_lock);
	return ret;
}
...
struct dentry *d_lookup(const struct dentry *parent, const struct qstr *name)
{
	...
	do {
		...
		if (dentry)
			break;
	} while (komb_read_seqretry(&rename_lock, seq));
	return dentry;
}
...
retry:
	...
	r_seq = komb_read_seqbegin(&rename_lock);
	...
	if (unlikely(komb_read_seqretry(&rename_lock, r_seq))) {
		rcu_read_unlock();
		goto retry;
	}
		     ...
		     bool exchange)
{
	...
	if (IS_ROOT(dentry)) {
		...
		spin_lock(&target->d_parent->d_lock);
		spin_lock_nested(&old_parent->d_lock,
 ...
 */
static int __d_unalias(struct inode *inode, struct dentry *dentry,
		       struct dentry *alias)
{
