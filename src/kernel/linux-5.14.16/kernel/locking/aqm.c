...
{
	if (cmpxchg(&node->lstatus, _AQ_MCS_STATUS_PWAIT, state) ==
	...
	/**
         ...
         **/
	if (cmpxchg(&node->lstatus, _AQ_MCS_STATUS_PARKED, state) ==
	...
	for (;;) {
		...
		if (curr->nid == nid) {
			...
			if (prev->nid == nid) {
				...
				woke_up_one = !force_update_node(
					curr, _AQ_MCS_STATUS_UNPWAIT);
