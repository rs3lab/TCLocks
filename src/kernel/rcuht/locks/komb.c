...
{
	return (node->socket_id == IRQ_NUMA_NODE || node->rsp == ((void*)0xdeadbeef));
}
...
#if PREFETCHING
			void *rsp_ptr =
				(void*)(per_cpu_ptr(&komb_nodes[0], next_node->cpuid)
					 ->rsp);
			prefetchw(rsp_ptr);
