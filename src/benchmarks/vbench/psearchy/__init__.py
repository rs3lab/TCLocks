...
class PsearchyLoad(Task, ResultsProvider):
    ...
    def wait(self, m):
        ...
        #cmd = [os.path.join(self.psearchyPath, "query", "qe"),
        #       "-t", self.dbPath,
        #       "-c", str(self.cores),
        #       "-q", "main",
        #       "-l"]
...
class Mkfiles(Task):
    ...
    def start(self):
        ...
        test = self.host.r.run(["test", "-f", self.filesPath], wait = UNCHECKED)
        if test.getCode() != 0:
            # Yes
            with Progress("Generating files list"):
                cmd = [os.path.join(self.psearchyPath, "mkfiles"), self.baseDir]
                self.host.r.run(cmd, stdout = self.filesPath + ".tmp", noCheck = True)
                self.host.r.run(["mv", self.filesPath + ".tmp", self.filesPath])
