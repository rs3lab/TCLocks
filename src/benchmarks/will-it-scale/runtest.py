...
data_points = json.loads(os.environ['python_env_cores'])
duration= int(os.environ['runtime'])
...
for i in data_points:
    perf_cmd = '' 
    c = perf_cmd + './build/%s_threads -t %d -s %d -n %d' % (cmd, i, duration, cores_per_socket)
    before = linux_stat()
